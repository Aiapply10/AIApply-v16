...
async def _fetch_from_jsearch(
    ...
    try:
        ...
        # Build query with remote filter if needed
        search_query = query
        if remote_only:
        ...
        if employment_type:
            ...
            if employment_type.upper() in emp_type_map:
                params["employment_types"] = emp_type_map[employment_type.upper()]
            ...
            for job in job_list[:per_page]:
                try:
                    ...
                    if job.get("job_min_salary") and job.get("job_max_salary"):
                        ...
                        salary_info = job.get("job_salary_period")
...
async def get_live_jobs_1_details(job_id: str, request: Request):
    """
    Get detailed job information from JSearch.
    """
    await get_current_user(request)
    ...
    try:
        # Remove the jsearch_ prefix if present
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            return {
                ...
                "highlights": job.get("job_highlights"),
                "apply_link": job.get("job_apply_link"),
                "salary_info": f"${job.get('job_min_salary', 0):,} - ${job.get('job_max_salary', 0):,}" if job.get("job_min_salary") else None,
                "is_remote": job.get("job_is_remote", False),
...
async def get_live_jobs_1_status(request: Request):
    """
    Get Live Jobs 1 (JSearch) API status.
    """
    await get_current_user(request)
    ...
    return {
        ...
        "api_name": "JSearch RapidAPI",
        "sources": ["Indeed", "LinkedIn", "Glassdoor", "ZipRecruiter", "SimplyHired", "More"],
        "features": ["remote_filter", "employment_type", "date_posted", "salary_info"]
    }

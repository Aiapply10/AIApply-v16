    ...
    <DashboardLayout>
      <PageTransition>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {/* Live Jobs 1 Tab - JSearch API */}
          <TabsContent value="livejobs1" className="mt-6">
            <Card className="mb-6">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Globe className="w-5 h-5 text-blue-500" />
                  Live Jobs 1 - Premium Search
                </CardTitle>
                <CardDescription>
                  Search jobs from Indeed, LinkedIn, Glassdoor, ZipRecruiter, and more
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleLiveJobs1Search} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="space-y-2">
                      <Label>Job Title / Keywords</Label>
                      <Input
                        placeholder={user?.primary_technology || "e.g., Python Developer"}
                        value={liveJobs1Form.query}
                        onChange={(e) => setLiveJobs1Form({...liveJobs1Form, query: e.target.value})}
                        data-testid="livejobs1-query"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        value={liveJobs1Form.location}
                        onChange={(e) => setLiveJobs1Form({...liveJobs1Form, location: e.target.value})}
                        data-testid="livejobs1-location"
                      />
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select 
                        value={liveJobs1Form.date_posted} 
                        onValueChange={(v) => setLiveJobs1Form({...liveJobs1Form, date_posted: v})}
                      >
                        <SelectTrigger data-testid="livejobs1-dateposted">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Switch
                        checked={liveJobs1Form.remote_only}
                        onCheckedChange={(checked) => setLiveJobs1Form({...liveJobs1Form, remote_only: checked})}
                        data-testid="livejobs1-remote"
                      />
                      <Label className="text-sm">Remote Jobs Only</Label>
                    </div>
                    
                    <Button type="submit" disabled={liveJobs1Loading} data-testid="livejobs1-search-btn">
                      {liveJobs1Loading ? (
                        <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                      ) : (
                        <Search className="w-4 h-4 mr-2" />
            ...
            {liveJobs1.length > 0 ? (
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <Globe className="w-4 h-4" />
                  Results from Indeed, LinkedIn, Glassdoor, ZipRecruiter & more
                </div>
                {liveJobs1.map((job) => (
                  <JobCard key={job.job_id} job={job} showSource />
                ))}
              </div>
            ) : (
              <Card className="border-dashed border-2">
                <CardContent className="flex flex-col items-center justify-center py-16">
                  ...
                  <h3 className="font-heading text-xl font-semibold mb-2">Premium Job Search</h3>
                  <p className="text-muted-foreground text-center max-w-md mb-4">
                    Search across Indeed, LinkedIn, Glassdoor, ZipRecruiter and more job platforms.
                  </p>
                  <Button 
                    onClick={() => handleLiveJobs1Search()}
                    disabled={liveJobs1Loading}
                  >
                    {liveJobs1Loading ? (
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                    ) : (
